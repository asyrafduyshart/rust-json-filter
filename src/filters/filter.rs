...
///     field: Some("age"),
///     operator: ">",
///     value: Some(Value::Number(Number::from(25))),
///     value_field: None,
...
///
/// ```
/// let filter_string = "age*2 > 30 AND .name = 'John Doe'";
/// let filters = parse(filter_string);
/// assert!(filters.is_some());
/// let filters = filters.unwrap();
/// assert_eq!(filters.len(), 2);
/// assert_eq!(filters[0].field, Some("age"));
/// assert_eq!(filters[0].operator, ">");
/// assert_eq!(filters[0].value.unwrap(), Value::Number(Number::from(30)));
/// assert_eq!(filters[1].field, Some("name"));
/// assert_eq!(filters[1].operator, "=");
/// assert_eq!(filters[1].value.unwrap(), Value::String("John Doe".to_string()));
/// ```
...
///
/// ```
/// let v = serde_json::json!({ "name": "John Doe", "age": 30 });
/// let filters = vec![
///     Filter { field: Some("name".to_string()), operator: "=".to_string(), value: Some("John Doe".to_string()), ..Default::default() },
///     Filter { field: Some("age".to_string()), operator: ">".to_string(), value: Some(25), ..Default::default() },
/// ];
/// assert_eq!(apply(&v, &filters), true);
/// ```
